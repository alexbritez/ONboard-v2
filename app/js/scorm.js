  ...
  if (!g_bInitDone) {
    if (!APIOK()) {
      ...
      if (err == "true") {
        ...
        if (!g_bSCOBrowse) {
          //alert('g_objAPI.GetValue("cmi.completion_status"):'+g_objAPI.GetValue("cmi.completion_status"));
          if (g_objAPI.GetValue("cmi.completion_status") == "not attempted" || g_objAPI.GetValue("cmi.completion_status") == "unknown") {
            //alert("set incomplete");
            err = g_objAPI.SetValue("cmi.completion_status","incomplete");
            //alert("err:"+err);
            //alert('g_objAPI.GetValue("cmi.completion_status"):'+g_objAPI.GetValue("cmi.completion_status"));
            //alert('cmi.success_status:'+g_objAPI.GetValue("cmi.success_status"));
          }
        }
        //alert('g_objAPI.GetValue("cmi.location"):'+g_objAPI.GetValue("cmi.location"));
      } else {
...
function SCOSetValue(nam,val){
  ...
  SCOCommit();
  if(nam=="cmi.core.location" || (nam=="cmi.core.success_status") || (nam=="cmi.score.scaled")){
    SCOCommit();
    SCOGetLastError();
  }
  return err
...
function exitFunction(){
  /*window.parent.Control.HideExitDialog();
  window.parent.Control.ReturnToLms('exit_all');*/
...
function playMovie() {
  getFlashMovie("index").JStoASviaExternalInterface('"'+SCOStart()+'"');
}

function SCOStart(){
  return SCOGetValue("cmi.location");
}


function SCOFinish(slide, complete, score){
  if ((APIOK()) && (g_bFinishDone == false)) {
    SCOReportSessionTime()
    SCOSetValue('cmi.location', ONboard.currentScene);
    SCOSetValue("cmi.progress_measure","1.00");
    SCOSetValue("cmi.score.min",0);
    SCOSetValue("cmi.score.max",100);

    if(ONboard.complete == true){
      SCOSetValue("cmi.score.raw",100);
      SCOSetValue("cmi.score.scaled",1);
      ...
      SCOSetValue("cmi.score.raw",0);
      SCOSetValue("cmi.score.scaled",0);

      SCOSetValue("cmi.completion_status","incomplete");
      SCOSetValue("cmi.success_status","unknown");
      SCOSetValue("cmi.suspend_data","");
      SCOSetValue("cmi.exit","suspend");
    }
    SCOCommit();
